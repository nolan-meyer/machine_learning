...
#OLS recipe
lm_rec <- recipe(Rating ~ Height + Age + Weight + Experience + College + hsloc, data = qb_comb) %>% 
...
#LASSO recipe
lasso_rec <- recipe(Rating ~ Height + Age + Weight + Experience + College + hsloc, data = qb_comb) %>%
    step_filter(`Passes Attempted` >= 50, `Games Played` > 0) %>% #removes potential outliers with few passes
