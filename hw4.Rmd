...
Here we show the output (model coefficients) from our trained logistic regression model. Then, we use a dot-and-whisker plot to examine the odd ratios represented by the exponentiated coefficient each predictor variable. 
...
of sensitivity, specificity, and accuracy, they aren't enough to really

### Classification - Conclusions

As previously mentioned, both random forest and logistic regression models 
perform relatively well on the data. 
...
As we can see once again, there is no real differentiating metric that shows
which model is superior. Both models are relatively strong, performing with an
